...
class DocumentIntegritySb:
    ...
    def extract_internal_legal_links(self, base_url: str, html: str):
        links = set()
    ...
    def check_document_html_language_content_and_links(self, case):
        ...
        canonical_url = self.normalize_url(urljoin(response.url, canonical))
        ...
        text = self.strip_html_text(html)
        assert len(text) > 300, f"Document text is too short: {case['url']}"
        assert not self.CYRILLIC_RE.findall(text), f"Cyrillic words detected in document: {case['url']}"
        ...
        has_version_signal = bool(self.YEAR_RE.search(text_lower)) or any(
        ...
        internal_legal_links = self.extract_internal_legal_links(response.url, html)
