...
class LiveApiContractSb:
    ...
    @staticmethod
    def require_staging_host(request):
    ...
    @staticmethod
    def payload_for(endpoint_path: str):
        stamp = int(time.time())
        phone = "35799112233"
    ...
    def check_contact_post_contract_live_staging(self, request, endpoint_path):
        api_base = self.require_staging_host(request)
        url = f"{api_base}{endpoint_path}"

        payload = self.payload_for(endpoint_path)
